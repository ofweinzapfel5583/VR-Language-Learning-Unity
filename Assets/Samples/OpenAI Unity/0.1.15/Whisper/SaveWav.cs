...
public static class SaveWav {
	...
	public static byte[] Save(string filename, AudioClip clip) {
		...
		Debug.LogError("Were about to call the persistent data path!!!!!!!!!!!!!!!");

		// Obtain the persistent data path
		string persistentDataPath = Application.persistentDataPath + "./Data/";

		// Log the persistent data path
		Debug.Log("Persistent Data Path: " + persistentDataPath);


		var filepath = Path.Combine(persistentDataPath, filename);

		Debug.Log("Saving WAV file to: " + filepath);
		Debug.LogError("Oh dang were about to write the file!!!!!!!!!!!!!!!");
		...
		using (var memoryStream = CreateEmpty(filepath))
		{
			...
			Debug.LogError("Alright the file is written!!!!!!!!!!!!!!!");
